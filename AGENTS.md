...
- **模式行恒显示，是本机信息块第一行**（`TransmuterGoggles#appendModeLine`，全类唯一输出点，已从 `append` 剥离且 `Readout` 去掉 mode 字段 → 不会两行）。停机时**照样**显示模式行，其下才是 `goggles.stellar_wave_transmuter_idle`（"未接入应力"）。`GoggleOverlayRenderer` 用**同一个 List** 先后调 `addToGoggleTooltip` 与 `addToTooltip`（中间插 `CommonComponents.EMPTY`、结尾 `remove(size-1)`），所以 BE 在 `addToTooltip` 里用 `tooltip.isEmpty()` 去重是正确判据；**无护目镜时前者根本不调**（条件是 `isWearingGoggles && instanceof IHaveGoggleInformation`），这时模式行由 `addToTooltip` 补出，且它的返回值必须 `|| modeAdded` 兜住，否则 Create 侧返回 false 会让整块提示被丢弃。
