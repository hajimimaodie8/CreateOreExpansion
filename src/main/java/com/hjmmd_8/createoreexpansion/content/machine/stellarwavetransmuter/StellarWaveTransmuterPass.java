 ...
 * （这里说的是<b>波的飞行方向</b>：竖直行进的波一律按撞墙处理，与机器怎么摆无关。
 * 机器侧"哪些面算波口"是另一回事——按机器自己的 4 个侧面判定，见
 * {@code StellarWaveTransmuterBlock#isOpenable}。）
...
public final class StellarWaveTransmuterPass {
	...
	public static Result tryConvert(AbstractChargerWaveEntity wave, BlockPos pos) {
		...
		if (Math.abs(wave.getMovement().y) > 0.2d)
			return Result.HIT_WALL;
		...
		Direction inDir = Direction.getNearest(wave.getMovement().x, 0, wave.getMovement().z);
		...
		Vec3 exit = Vec3.atCenterOf(pos)
			.add(wave.getMovement().scale(0.65d));
		StellarWaveEntity variant = new StellarWaveEntity(level, exit, wave.getMovement(), wave.getWaveLevel());
		...
		variant.addSpeedOffset(wave.getSpeedOffset());
		variant.setCharge(wave.getChargePolarity());
		variant.setRenderColor(wave.getRenderColor());
